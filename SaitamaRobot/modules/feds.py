...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID:
        message.reply_text("Disaster level Devil cannot be fed banned!")
        return

    if int(user_id) in DRAGONS:
        message.reply_text("Butler cannot be fed banned!")
        return

    if int(user_id) in TIGERS:
        message.reply_text("General cannot be fed banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("Practitioner cannot be fed banned!")
        return
