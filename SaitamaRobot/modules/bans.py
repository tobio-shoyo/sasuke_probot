...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against one of the Devil Disaster huh?")
            ...
            message.reply_text(
                "Bring an order from Devil Kings to fight a Monarch disaster.",
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Devil Kings to fight a Knight disaster.",
            )
        elif user_id in WOLVES:
            message.reply_text("Practitioner's  abilities make them ban immune!")
        else:
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Kicked The Ass! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
