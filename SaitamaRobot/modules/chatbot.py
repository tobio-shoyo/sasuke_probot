...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_kuki:
        ...
        msg.reply_text("Sasuke Uchiha AI successfully enabled for this chat!")
        message = (
        ...
        return message
    msg.reply_text("Sasuke Uchiha AI is already enabled for this chat!")
    return ""
...
def rem_chat(update: Update, context: CallbackContext):
    ...
    is_kuki = sql.is_kuki(chat.id)
    if not is_kuki:
        msg.reply_text("Sasuke Uchiha AI isn't enabled here in the first place!")
        return ""
    sql.rem_kuki(chat.id)
    msg.reply_text("Sasuke Uchiha AI disabled successfully!")
    message = (
        f"<b>{html.escape(chat.title)}:</b>\n"
        f"AI_DISABLED\n"
        f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
__help__ = """
Chatbot utilizes the Kuki API and allows Sasuke to talk and provides a more interactive group chat experience.
