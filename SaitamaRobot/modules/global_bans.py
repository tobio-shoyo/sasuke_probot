...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is one of the DEVIL KINGS \nI can't act against our own.",
        )
        return
    ...
    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a Monarch Disaster! *grabs popcorn*",
        )
        return

    if int(user_id) in TIGERS:
        message.reply_text("That's a General ! They cannot be banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("That's a Practitioner ! They cannot be banned!")
        return
